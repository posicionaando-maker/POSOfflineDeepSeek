...
// ============================================
// MATADOR DE SERVICE WORKERS CORRUPTOS
// AL PRINCIPIO DE script.js, AGREGAR:
const FORZAR_DATOS_LOCALES = true;  // ← CAMBIAR A true PARA PRUEBA

// Luego modificar la función init():
async function init() {
    if (FORZAR_DATOS_LOCALES) {
        console.log('📦 Usando datos locales forzados');
        productos = [
        ...
        renderizarProductos();
        actualizarCarritoUI();
        mostrarToast('Modo demostración - Datos locales', 'info');
        return;
    }
...
(async function matarServiceWorkers() {
    if ('serviceWorker' in navigator) {
        try {
            ...
            console.log('✅ Todos los Service Workers han sido eliminados');
            
            // Recargar la página si había SW activo
...
function importarDesdeGoogle() {
    // ============================================
// SINCRONIZACIÓN MANUAL (Botón de flechas)
// ============================================
async function importarDesdeGoogle() {
    ...
    try {
        // Usar la URL fija que configuramos
        ...
        if (data.success && data.productos && data.productos.length > 0) {
            ...
            mostrarToast(`✅ ${data.productos.length} productos sincronizados`, 'success');
            console.log('✅ Sincronización exitosa:', data.productos.length, 'productos');
        } else {
            throw new Error(data.error || 'No se obtuvieron datos');
        }
        
    } catch (error) {
        console.error('❌ Error en sincronización:', error);
        container.innerHTML = originalContent;
        mostrarToast('Error al sincronizar. ¿Conexión a internet?', 'error');
        
        // Intentar cargar desde localStorage como respaldo
        const productosLocales = localStorage.getItem('productos');
        if (productosLocales) {
            productos = JSON.parse(productosLocales);
            renderizarCategorias();
            renderizarProductos();
            mostrarToast('Usando datos guardados localmente', 'warning');
        }
    }
}
